...
INSERT INTO `users`(`name`, `avatar`,`email`,`password`,`contacts`)
...
INSERT INTO `lots`(`name`,`description`,`id_user`,`id_category`,`id_winner`,`img`,`start_price`,`step_price`,`data_over`)
...
Select a.name,
       start_price,
       img,
       (sum(b.price)+a.start_price)Последняя_ставка,
       count(b.price)Количество_ставок,
       c.name,
       id_winner
from lots as a
       left join bids b
